...
(function () {
  ...
  function getOrders() { return (getStoreArray('customerOrders') || []).map(normalizeOrder).filter(function (o) { return o.status !== 'cancelled' && o.status !== 'fulfilled'; }); }
    ...
    <div class="modal fade" id="customerDdtModal" tabindex="-1" aria-hidden="true">
      <div class="modal-dialog modal-xl modal-dialog-scrollable"><div class="modal-content">
        ...
        <div class="modal-body"><form id="customerDdtForm" onsubmit="return false;">
          ...
          <div class="card mb-3 d-none" id="customerDdt-multiOrdersPanel"><div class="card-header fw-semibold d-flex justify-content-between align-items-center"><span>Ordini cliente da accorpare</span><span class="small text-muted">Stesso cliente, ordini aperti o parzialmente evasi</span></div><div class="card-body">
          ...
          <div class="card mb-3"><div class="card-header fw-semibold">Righe DDT</div><div class="card-body">
            <div class="row g-2 align-items-end" id="customerDdt-manualLineControls">
              ...
              <div class="col-md-2"><label class="form-label">Quantità consegnata</label><input class="form-control text-end" id="customerDdt-lineShippedQty" type="number" min="0" step="0.001" value="1" /></div>
  ...
  function renderOrderOptions() { const $sel = $('#customerDdt-sourceOrderId'); if (!$sel.length) return; const cur=$sel.val(); $sel.empty().append('<option value="">Nessun ordine collegato</option>'); getOpenOrders().forEach(function(o){ $sel.append('<option value="'+esc(o.id)+'">'+esc(orderLabel(o))+'</option>'); }); if (cur) $sel.val(cur); }
