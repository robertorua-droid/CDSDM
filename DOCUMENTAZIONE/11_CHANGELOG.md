## CDSDM Versione 0.0.1 — Migrazione nuovo progetto Firebase
- aggiornata la configurazione Firebase in `js/services/firebase-cloud.js` per collegare l'app al progetto `cdsdm-b6e8b`.
- mantenuto l'uso degli SDK Firebase compat già caricati da `index.html`; non è stato introdotto il formato modulare `import`.
- nessuna modifica alla struttura dati applicativa: Firestore continua a usare il ramo per utente `users/{uid}` e le collezioni già esistenti.
- aggiornata la schermata **Versione** e rigenerata la documentazione in-app.
